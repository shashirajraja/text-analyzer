...
function Alert(props) {
    ...
    const alertClassName = `alert alert-${type} alert-dismissible fade show pt-0 pb-0 mt-1 mb-1`
    return message && (
    <>
        <div className={alertClassName} role="alert">
            {/* <strong><strong>{convertToTitleCase(type)}</strong>: {message}</strong> */}
            <strong>{message}</strong>
            <button type="button" className="close mx-md-2" data-dismiss="alert" aria-label="Close">
